...
from src.agents.domain_agents import build_default_agents
...
class AgentRegistry:
    ...
    def __init__(self, repo: Any = None, shared_state: SharedAgentState | None = None) -> None:
        self._repo = repo
    ...
    def _aggregate(
        ...
        confidence = max(
            0.0, min(0.99, avg_confidence - conflict_penalty + (0.05 if successful else 0.0))
        )
        ...
        return CollaborationAggregation(
            success=success,
            summary=summary,
    ...
    async def dispatch_task(self, task: str, target_agent: str = "") -> dict[str, Any]:
        ...
        plan = (
            plan_result.data.get("collaboration_plan", {})
                        ...
                        "agent_results": [
                            {
                                ...
                                "payload": {"data": result.data, "summary": result.summary},
                            }
                            for result in results
                        ],
                        "conflicts": conflicts,
    ...
    async def fan_out(self, task: str) -> list[AgentResult]:
        ...
        return await self._run_agents(
            agents, task, shared_state, {"parallel_groups": [[agent.agent_id for agent in agents]]}
        )
    ...
    async def collaborate(self, agents: list[str], task: str) -> dict[str, Any]:
        ...
        results = await self._run_agents(
            ...
            task,
            shared_state,
            {"parallel_groups": [[agent.agent_id for agent in selected_agents]]},
        )
        conflicts = self._resolve_conflicts(results)
        aggregation = self._aggregate(task, results, conflicts, (time.time() - start) * 1000)
    ...
    def _update_shared_state(
        self, aggregation: CollaborationAggregation, primary_result: AgentResult | None, task: str
    ) -> None:
