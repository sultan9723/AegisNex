...
class AutonomousPipeline:
    ...
    async def _on_incident_created(self, event: Event) -> None:
        ...
        service = payload.get("service_name") or payload.get("incident", {}).get("service_name", "unknown")
        desc = payload.get("description") or payload.get("incident", {}).get("description", "")
        await self.run_pipeline(incident_id=incident_id, service_name=service, description=desc, correlation_id=event.correlation_id)
    ...
    async def run_pipeline(
        ...
        exec_id = None
        if self._history:
            exec_id = self._history.start_execution(trigger=f"incident:{incident_id or '?'}", incident_id=incident_id)

        try:
            # Step 1: Plan — use supervisor agent
            result.steps_completed.append("plan")
            step_id = self._history.add_step("planning", "Incident planning") if self._history else None
            plan_result = None
            if self._agents:
                plan_result = await self._agents.dispatch_task(f"Investigate and resolve: {description}", "supervisor-agent")
            ...
            step_id = self._history.add_step("evidence", "Evidence collection", input={"service": service_name}) if self._history else None
            ...
            if self._history:
                ...
                decision = {"step": "remediation_planning", "plan": plan, "evidence_count": len(evidence)}
                self._history.add_decision(decision)

            # Step 5: Risk assessment & approval
            result.steps_completed.append("risk_assessment")
            step_id = self._history.add_step("approval", "Risk assessment & approval", input={"plan": plan}) if self._history else None
            ...
            step_id = self._history.add_step("execution", "Remediation execution", input={"plan": plan}) if self._history else None
            ...
            explanation = self._build_explanation(service_name, description, plan, verification, result.root_cause)
            ...
            await self._update_knowledge(service_name, description, result.root_cause or "", plan, verification)

            result.status = "completed"
            await self._bus.publish(
                EventType.WORKFLOW_COMPLETED,
                {"pipeline_id": pipeline_id, "incident_id": incident_id, "status": "completed", "explanation": explanation},
                source="autonomous_pipeline",
                correlation_id=correlation_id,
            )
    ...
    async def _collect_evidence(self, service_name: str, description: str) -> List[Dict[str, Any]]:
        evidence: List[Dict[str, Any]] = [
            {"type": "incident_description", "value": description, "source": "event_bus", "timestamp": _utc_now()},
            {"type": "service_name", "value": service_name, "source": "event_bus", "timestamp": _utc_now()},
        ]
        if self._agents:
            try:
                agent_result = await self._agents.dispatch_task(f"Investigate incident on {service_name}: {description}")
    ...
    async def _generate_plan(self, service_name: str, description: str, evidence: List[Dict[str, Any]]) -> Dict[str, Any]:
        plan: Dict[str, Any] = {
        ...
        if self._agents:
            try:
                agent_result = await self._agents.dispatch_task(f"Generate remediation plan for {service_name}: {description}")
    ...
    async def _check_approval(self, plan: Dict[str, Any]) -> bool:
        ...
        for action_item in actions:
            action_name = action_item if isinstance(action_item, str) else action_item.get("action", "")
    ...
    async def _execute_remediation(self, plan: Dict[str, Any], correlation_id: Optional[str] = None) -> Dict[str, Any]:
        results: Dict[str, Any] = {}
        actions = plan.get("actions", [])

        for action_item in actions:
            action_name = action_item if isinstance(action_item, str) else action_item.get("action", "")
            target = action_item.get("target", plan.get("service", "unknown")) if isinstance(action_item, dict) else plan.get("service", "unknown")

            if self._healing:
                healing_event = self._map_action_to_event(action_name)
                if healing_event:
                    healing_result = await self._healing.handle_event(healing_event, {"name": target, "target_type": "unknown"})
                    if healing_result:
                        results[action_name] = healing_result.to_dict()
                        await self._bus.publish(
                            EventType.REMEDIATION_COMPLETED if healing_result.status == "completed" else EventType.REMEDIATION_FAILED,
                            {"action": action_name, "target": target, "result": healing_result.to_dict()},
                            source="autonomous_pipeline",
                            correlation_id=correlation_id,
                        )
    ...
    async def _update_knowledge(self, service_name: str, description: str, root_cause: str, plan: Dict[str, Any], verification: Dict[str, Any]) -> None:
    ...
    async def _failsafe_rollback(self, pipeline_id: str, incident_id: Optional[str]) -> None:
        ...
        try:
            ...
            if self._incidents and incident_id:
                try:
                    self._incidents.update_incident(incident_id, remediation_attempted=True, remediation_successful=False)
                except KeyError:
                    pass
            await self._bus.publish(
                EventType.AUTONOMOUS_ACTION,
                {"action": "rollback", "pipeline_id": pipeline_id, "incident_id": incident_id, "status": "rolled_back"},
                source="autonomous_pipeline",
    ...
    def _map_action_to_event(self, action: str) -> Optional[EventType]:
        mapping = {
