...
class WorkflowLibrary:
    ...
    def find_by_trigger(self, trigger: str) -> builtins.list[WorkflowDef]:
        t = trigger.lower()
...
_GLOBAL_LIBRARY: WorkflowLibrary | None = None
...
def register_default_workflows(lib: WorkflowLibrary) -> None:
    lib.register(
        WorkflowDef(
            ...
            steps=[
                WorkflowStep(
                    name="check-http", description="Check HTTP endpoint status", tools=["target"]
                ),
                WorkflowStep(
                    name="check-containers",
                    description="Check web server containers",
                    tools=["docker"],
                ),
                WorkflowStep(
                    name="check-health",
                ...
                WorkflowStep(
                    ...
                    description="Verify site is back online",
                    tools=["target"],
                ),
            ],
        )
    ...
    lib.register(
        WorkflowDef(
            ...
            steps=[
                WorkflowStep(
                    name="collect-metrics", description="Collect CPU metrics", tools=["metrics"]
                ),
                WorkflowStep(
                    name="find-consumers",
                    description="Identify top CPU consumers",
                    tools=["docker"],
                    parallel=True,
                ),
                WorkflowStep(
                ...
                WorkflowStep(
                    name="remediate",
                    description="Run CPU remediation",
                    runbook="high-cpu",
                    requires_approval=True,
                ),
    ...
    lib.register(
        WorkflowDef(
            ...
            steps=[
                WorkflowStep(
                    name="inspect", description="Inspect container status", tools=["docker"]
                ),
                WorkflowStep(
                    name="restart",
    ...
    lib.register(
        WorkflowDef(
            ...
            steps=[
                WorkflowStep(
                    name="check-ssl", description="Check SSL certificate status", tools=["target"]
                ),
                WorkflowStep(
                    name="notify",
                    description="Send notification if expiring soon",
                    tools=["notification"],
                ),
            ],
        )
    ...
    lib.register(
        WorkflowDef(
            ...
            steps=[
                WorkflowStep(
                    name="check-health", description="Check database health", tools=["health"]
                ),
                WorkflowStep(
                    name="check-containers",
                    description="Check database containers",
                    tools=["docker"],
                ),
                WorkflowStep(
                    name="check-metrics",
                ...
                WorkflowStep(
                    name="remediate", description="Restart database service", requires_approval=True
                ),
                WorkflowStep(
                    name="verify", description="Verify database is back online", tools=["health"]
                ),
            ],
        )
    )
    ...
    lib.register(
        WorkflowDef(
            ...
            steps=[
                ...
                WorkflowStep(
                    name="verify", description="Verify disk space freed", tools=["metrics"]
                ),
            ],
        )
    )
    ...
    lib.register(
        WorkflowDef(
            ...
            steps=[
                WorkflowStep(
                    name="collect-metrics", description="Collect memory metrics", tools=["metrics"]
                ),
                WorkflowStep(
                    name="find-consumers", description="Identify memory consumers", tools=["docker"]
                ),
                WorkflowStep(
                    name="check-health",
                    description="Check system health",
                ...
                WorkflowStep(
                    ...
                    description="Restart memory-heavy container",
                    requires_approval=True,
                ),
                WorkflowStep(
                    name="verify", description="Verify memory usage normalized", tools=["metrics"]
                ),
            ],
        )
    )
