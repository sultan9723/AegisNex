...
class SQLiteMemoryStore(MemoryStore):
    ...
    def get_integrations(self) -> list[dict[str, Any]]:
        with self._lock:
            rows = (
                self._conn()
                .execute("SELECT * FROM ai_integrations ORDER BY created_at DESC")
                .fetchall()
            )
            integrations = []
            for r in rows:
        ...
        with self._lock:
            cur = self._conn().execute(
                ...
                (
                    ...
                    severity,
                    service,
                    extra.pop("status", "open"),
                    1 if extra.pop("resolved", False) else 0,
                    json.dumps(extra),
                ),
            )
            self._conn().commit()
    ...
    def store_recommendation(
        self, request: str, recommendation: str, confidence: float = 0.0, **extra: Any
    ) -> int:
        with self._lock:
            cur = self._conn().execute(
    ...
    def store_remediation(
        self, action: str, target: str, successful: bool = False, **extra: Any
    ) -> int:
        with self._lock:
            cur = self._conn().execute(
    ...
    def _search_table(self, table: str, query: str, limit: int = 10) -> MemorySearchResult:
        ...
        conditions = " OR ".join(
            f"({' OR '.join(f'LOWER({c}) LIKE ?' for c in cols)})" for _ in terms
        )
        params = []
        for t in terms:
            like = f"%{t}%"
        ...
        with self._lock:
            conn = self._conn()
            row = conn.execute(
                f"SELECT COUNT(*) as cnt FROM {table} WHERE {conditions}", params
            ).fetchone()
    ...
    def get_recent_learnings(self, limit: int = 10) -> list[dict[str, Any]]:
        with self._lock:
            rows = (
                self._conn()
                .execute("SELECT * FROM ai_learnings ORDER BY created_at DESC LIMIT ?", [limit])
                .fetchall()
            )
            return [dict(r) for r in rows]

    def search_tool_executions(self, query: str, limit: int = 10) -> MemorySearchResult:
        return self._search_table("ai_tool_executions", query, limit)
    ...
    def search_all(self, query: str, limit: int = 10) -> MemorySearchResult:
        ...
        combined.entries = sorted(
            combined.entries, key=lambda x: x.get("created_at", ""), reverse=True
        )[:limit]
    ...
    def get_recent_conversations(self, limit: int = 10) -> list[dict[str, Any]]:
        with self._lock:
            rows = (
                self._conn()
                .execute("SELECT * FROM ai_conversations ORDER BY created_at DESC LIMIT ?", [limit])
                .fetchall()
            )
            return [dict(r) for r in rows]
    ...
    def get_recent_incidents(self, limit: int = 10) -> list[dict[str, Any]]:
        with self._lock:
            rows = (
                self._conn()
                .execute("SELECT * FROM ai_incidents ORDER BY created_at DESC LIMIT ?", [limit])
                .fetchall()
            )
            return [dict(r) for r in rows]
    ...
    def get_recent_remediations(self, limit: int = 10) -> list[dict[str, Any]]:
        with self._lock:
            rows = (
                self._conn()
                .execute("SELECT * FROM ai_remediations ORDER BY created_at DESC LIMIT ?", [limit])
                .fetchall()
            )
            return [dict(r) for r in rows]
    ...
    def store_knowledge_doc(
        ...
        with self._lock:
            existing = (
                self._conn()
                .execute("SELECT id FROM ai_knowledge_docs WHERE source = ?", (source,))
                .fetchone()
            )
            if existing:
                self._conn().execute(
                ...
                self._conn().execute(
                    "DELETE FROM ai_knowledge_chunks WHERE doc_id = ?", (existing["id"],)
                )
                self._conn().commit()
                return existing["id"]
            cur = self._conn().execute(
    ...
    def get_knowledge_doc_by_source(self, source: str) -> dict[str, Any] | None:
        with self._lock:
            row = (
                self._conn()
                .execute("SELECT * FROM ai_knowledge_docs WHERE source = ?", (source,))
                .fetchone()
            )
    ...
    def list_knowledge_docs(self) -> list[dict[str, Any]]:
        with self._lock:
            rows = (
                self._conn()
                .execute("SELECT * FROM ai_knowledge_docs ORDER BY indexed_at DESC")
                .fetchall()
            )
            return [dict(r) for r in rows]
    ...
    def get_knowledge_stats(self) -> dict[str, Any]:
        with self._lock:
            ...
            sources = conn.execute(
                "SELECT source FROM ai_knowledge_docs ORDER BY indexed_at DESC"
            ).fetchall()
            return {
    ...
    def get_stats(self) -> dict[str, Any]:
        with self._lock:
            ...
            row = conn.execute(
                "SELECT COUNT(*) as cnt FROM ai_conversations WHERE goal_achieved = 1"
            ).fetchone()
            stats["successful_conversations"] = row["cnt"] if row else 0
            row = conn.execute(
                "SELECT COUNT(*) as cnt FROM ai_remediations WHERE successful = 1"
            ).fetchone()
            stats["successful_remediations"] = row["cnt"] if row else 0
            row = conn.execute(
                "SELECT AVG(duration_ms) as avg_dur FROM ai_tool_executions"
            ).fetchone()
            stats["avg_tool_duration_ms"] = (
                round(row["avg_dur"], 2) if row and row["avg_dur"] else 0.0
            )
            row = conn.execute(
                "SELECT AVG(confidence) as avg_conf FROM ai_conversations"
            ).fetchone()
