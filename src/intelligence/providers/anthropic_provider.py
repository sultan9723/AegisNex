...
class AnthropicProvider(ModelProvider):
    ...
    def chat(self, messages: list[Message], **kwargs: Any) -> Message:
        ...
        resp = self._client.messages.create(
            ...
            messages=[
                MessageParam(role=m.role, content=m.content) for m in messages if m.role != "system"
            ],
        )
    ...
    def embed(self, text: str, **kwargs: Any) -> list[float]:
        raise NotImplementedError(
            "Anthropic does not provide a standalone embedding API through this SDK path"
        )
