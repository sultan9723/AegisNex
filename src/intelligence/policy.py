...
def _utc_now() -> str:
    ...
    _UTC_NOW_CACHE = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
    ...
    def check_action(
        ...
        for policy in self._policies:
            ...
            if policy.effect == "deny":
                return PolicyResult(allowed=False, policy_name=policy.name, reason=policy.description)
            elif policy.effect == "require_approval":
                return PolicyResult(allowed=True, policy_name=policy.name, reason=policy.description, requires_approval=True, suggested_action="Request human approval")
            elif policy.effect == "allow":
                return PolicyResult(allowed=True, policy_name=policy.name, reason=policy.description)
