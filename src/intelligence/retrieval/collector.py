...
from src.intelligence.retrieval.base import SourceDocument


class KnowledgeCollector:
    def __init__(self, repo: Any = None, runbooks_dir: str | None = None) -> None:
        self._repo = repo
        ...
        try:
            ...
            for inc in raw[:limit]:
                ...
                docs.append(
                    SourceDocument(
                        content=text,
                        source=f"incident:{inc.get('incident_id', '?')}",
    ...
    def collect_runbooks(self, query: str, limit: int = 5) -> list[SourceDocument]:
        ...
        for fp in files:
            try:
                ...
                score = sum(1 for t in terms if t in content.lower() or t in title.lower()) / max(
                    len(terms), 1
                )
                if score > 0:
                    docs.append(
                        SourceDocument(
                            ...
                            source=f"runbook:{fp.name}",
                            source_type="runbook",
                            relevance_score=score,
                            metadata={"path": str(fp), "title": title},
                        )
                    )
            except Exception:
                continue
