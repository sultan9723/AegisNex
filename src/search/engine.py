...
def _make_result(
    ...
    route = (
        route_template.format(id=rid)
        if route_template
        else f"/{DOMAIN_MAP.get(domain, {}).get('route', domain)}"
    )
    return SearchResult(
        domain=domain,
        id=rid,
...
class SearchEngine:
    ...
    def search(
        self, query: str, domain: str = "all", limit: int = 20, filters: dict | None = None
    ) -> SearchResults:
