...
function downloadEvidencePacket(incidentId: string, packet: EvidencePacket) {
  ...
  anchor.href = url;
  anchor.download = `${incidentId}-client-evidence.json`;
  anchor.click();
...
export default function ClientEvidencePage({ params }: { params: { id: string } }) {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const [clientResult, statsResult, incidentResult, approvalResult] = await Promise.all([
        getClientOrganization(orgId),
        getClientOrgStats(orgId),
        getIncidents(1000, undefined, orgId),
        getApprovals({ limit: 100 }),
      ]);
      const assignedIncidents = incidentResult.incidents ?? [];
      const incidentIds = new Set(assignedIncidents.map((incident) => incident.incident_id));
      ...
      setError(err instanceof Error ? err.message : "Failed to load client evidence");
      setClient(null);
      setStats(null);
      setIncidents([]);
      setApprovals([]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Badge variant="secondary" size="sm">client evidence</Badge>
            <Badge variant={client?.is_active ? "success-subtle" : "secondary"} size="sm">
              {client?.is_active ? "active" : "inactive"}
            </Badge>
          </div>
          <h1 className="mt-3 text-2xl font-bold tracking-tight text-text-primary">{client?.name || "Client"}</h1>
          <p className="mt-1 max-w-2xl text-sm text-text-secondary">
            Incidents, approvals, and collected diagnostic evidence assigned to this managed client.
          </p>
          {client?.domain && <p className="mt-1 text-xs text-text-tertiary">{client.domain}</p>}
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatTile label="Total incidents" value={incidents.length} icon={Building2} />
      ...
      <section className="rounded-xl border border-border bg-surface p-5">
        <div className="mb-4 flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-0.5 text-xs text-text-tertiary">Collected read-only diagnostics for this client.</p>
          </div>
        ...
        {evidencePackets.length === 0 ? (
          <EmptyState
            ...
            description="Evidence will appear here after an assigned incident completes the approved diagnostics workflow."
          />
        ) : (
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        <div className="rounded-xl border border-border bg-surface p-5">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-base font-semibold text-text-primary">Assigned Incidents</h2>
            <Badge variant="secondary" size="sm">{incidents.length}</Badge>
          </div>
          {incidents.length === 0 ? (
            <p className="py-8 text-center text-sm text-text-tertiary">No incidents assigned to this client.</p>
          ) : (
        ...
        <div className="rounded-xl border border-border bg-surface p-5">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-base font-semibold text-text-primary">Client Approvals</h2>
